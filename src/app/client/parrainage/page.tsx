...
export default async function ClientParrainagePage() {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-slate-400">
          Invitez d&apos;autres entreprises sur QUANTUM et gagnez 200 XP par parrainage validé.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Gift className="h-5 w-5 text-quantum-accent" />
        ...
        <CardContent>
          {referralCode ? (
            <div className="space-y-4">
              <div className="flex items-center gap-3 rounded-lg border border-quantum-accent/30 bg-quantum-accent/5 px-4 py-3">
                <p className="flex-1 font-mono text-xl font-bold tracking-widest text-quantum-accent">
                  {referralCode}
                </p>
                <CopyReferralButton code={referralCode} />
              </div>
              <p className="text-sm text-slate-400">
                Partagez ce code aux entreprises que vous souhaitez inviter. Elles devront le saisir lors de leur inscription.
                Vous recevrez 200 XP dès que leur compte sera validé par notre équipe.
              </p>
            </div>
          ) : (
            <p className="text-slate-400">
              Votre code de parrainage sera généré prochainement. Contactez l&apos;équipe QUANTUM si ce n&apos;est pas le cas après 24h.
            </p>
          )}
        </CardContent>
      </Card>
            ...
            <ul className="divide-y divide-white/5">
              {referrals.map((r, i) => (
                <li key={r.id} className="flex items-center justify-between py-3">
                  <div>
                    <p className="text-sm font-medium text-white">Entreprise #{i + 1}</p>
